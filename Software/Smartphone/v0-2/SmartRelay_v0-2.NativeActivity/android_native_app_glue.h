 ...
 * Consulte a Licença para permissões de governo específicas e
 ...
 * é direto, leve, mas tem restrições.
 *
...
struct android_poll_source {
    ...
    // Função a ser chamada para executar o processamento padrão de dados a partir
    // dessa fonte.
...
struct android_app {
    ...
    // Preencha com a função para processar os principais comandos do aplicativo (APP_CMD_*)
    ...
    // o retorno.  Retornar 1 se você manuseou o evento, 0 para qualquer expedição
    // padrão.
    int32_t (*onInputEvent)(struct android_app* app, AInputEvent* event);

    // A instância do objeto ANativeActivity na qual este aplicativo está executando.
    ANativeActivity* activity;

    // A configuração atual na qual o aplicativo está executando.
    ...
    // no ponto em que serão inicializados para NULO e você pode fazer malloc de seu
    ...
    // destruído e aguardando o thread de aplicativos ser concluído.
...
enum {
     ...
     * é retornado como um identificador de ALooper_pollOnce().  O dado para este
     * identificador é um ponteiro para uma estrutura android_poll_source.
     ...
     * ALooper_pollOnce().  O dado para este identificador é um ponteiro para uma estrutura
 ...
 * ele será definido como NULO.
     ...
     * como na janela de entrada simples que está sendo exibida ou ocultada.  Você pode
